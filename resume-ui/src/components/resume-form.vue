<template>
  <el-form :model="data" label-width="auto">
    <el-form-item v-for="formItem in formItems" :label="formItem.name||formItem.label">
      <!--输入框-->
      <el-input v-if="formItem.type==='input'" v-model="data[`${formItem.label}`]"></el-input>

      <!--单选-->
      <el-radio-group v-model="data[`${formItem.label}`]" v-else-if="formItem.type==='radio'">
        <el-radio v-for="dict in formItem.dict.split(',')" :value="dict">{{ dict }}</el-radio>
      </el-radio-group>
      <!--下拉单选-->
      <el-select v-else-if="formItem.type==='select'" v-model="data[`${formItem.label}`]">
        <el-option v-for="dict in formItem.dict.split(',')" :label="dict"
                   :value="dict"></el-option>
      </el-select>
      <!--多选-->
      <el-checkbox-group v-else-if="formItem.type==='checkbox'" v-model="data[`${formItem.label}`]">
      ...
      <!--富文本-->
      <editor v-model:get-html="data[`${formItem.label}`]" v-else-if="formItem.type==='textarea'"></editor>
